/**
 * YouTube URL handling for the Gallery field (server copy).
 *
 ...
 *
 * Mirror of src/shared/utils/youtube.ts, which is the source of truth — the
 * Cloud Functions build cannot import from src/. A parity spec runs the same
 * table of URLs through both, so a change here without one there fails the
 * suite rather than publishing pages that disagree with the preview.
 *
