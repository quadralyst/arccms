...
6. **Member-doc delete closes the mirror (gap found while building erasure):** `onWaitlistUserDeleted` removes the contact from `waitlist-{id}` via `removeContactFromLists`. Create, verify and whole-form delete were all mirrored; deleting a *single* member doc (the joined-users page) was not, so the contact kept the listId and stayed in the list's audience and drip campaigns. Decisions, documented in `docs/email-system.md` §9: it removes **only the form's own list** (a form feeds `targetListIds`, it does not own them), it **never deletes the contact** (erasure is `adminDeleteContact` alone — a member-doc delete must not silently destroy an address), and it **stands down when the parent form doc is gone** so it cannot race `onWaitlistsDelete` into resurrecting a deleted list doc with a negative `memberCount`. Existing deployments carry drift from before this trigger: contacts whose member docs were already deleted keep the stale membership, and no callable repairs them yet.
