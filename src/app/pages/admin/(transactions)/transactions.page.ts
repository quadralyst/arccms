                ...
                <mat-card-content class="pt-3">
                    @if (store.isLoading()) {
                        ...
                        <table mat-table [dataSource]="filtered()" class="w-100">
                            ...
                            <ng-container matColumnDef="status"><th mat-header-cell *matHeaderCellDef>{{ 'common.table.status' | transloco }}</th><td mat-cell *matCellDef="let t"><span class="status status-{{ t.status }}">{{ t.status }}</span>@if (t.isTest) {<span class="status status-test" [title]="'admin.transactions.test_badge_hint' | transloco">{{ 'admin.transactions.test_badge' | transloco }}</span>}</td></ng-container>
